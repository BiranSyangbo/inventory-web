...
const ENTITIES = [
    {
        ...
        color: 'blue',
        columns: ['name*', 'brand', 'category', 'volume_ml', 'type', 'alcohol_percentage', 'mrp', 'min_stock', 'selling_price*', 'status'],
        example: 'Royal Stag,Seagram,Whiskey,750,Full,34.8,230, 1,520.00,ACTIVE',
    },
    {
        key: 'customers',
        label: 'Customers',
                ...
                {ENTITIES.map(ent => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <span
                                className={`text-xs ${active ? 'text-white/70' : 'text-slate-500'}`}>{ent.description}</span>
                        </button>
                    )
                })}
            </div>
                            ...
                            >
                                ...
                                {file ? (
                                    ...
                                    <>
                                        ...
                                        <div className="text-center">
                                            <p className="text-slate-300 text-sm font-medium">Drop file here or click to
                                                browse</p>
                ...
                <div className="lg:col-span-2 space-y-4">
                    <div className="bg-slate-800 rounded-xl border border-slate-700 p-5">
                        ...
                        <div className="space-y-3">
                            <div>
                                <p className="text-xs text-slate-400 mb-2">
                                    Row 1 must be a <span className="text-slate-200 font-medium">header row</span>.
                                    Columns are positional — names are ignored.
                        ...
                        {i + 1}. {name}{required && <span className="text-red-400 ml-0.5">*</span>}
                            ...
                            {/* Example row */}
                            <div>
                                <p className="text-xs text-slate-500 mb-1.5 font-medium uppercase tracking-wide">Example
                                    row</p>
                                <div
                                    className="bg-slate-900 rounded-lg p-3 text-xs font-mono text-slate-300 break-all leading-relaxed border border-slate-700">
                                    {selected.example}
                                </div>
                            </div>
                    ...
                    <div className="bg-slate-800 rounded-xl border border-slate-700 p-5">
                        <div className="flex items-start gap-3">
                            ...
                            <div className="text-xs text-slate-400 space-y-1.5">
                                ...
                                <p>Failed rows are skipped — the rest are still processed. Always review the error table
                                    after import.</p>
                                {(entity === 'purchases' || entity === 'sales') && (
                                    <p className="text-slate-300">Stock side-effects are <strong
                                        className="text-white">immediate</strong> — purchases update average cost and
                                        increase stock; sales run FIFO batch allocation.</p>
                                )}
                            </div>
                        </div>
                    </div>
    ...
    return (
        <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700">
                ...
                {allOk
                    ? <span className="flex items-center gap-1.5 text-xs font-medium text-green-400"><CheckCircle
                        size={13}/> All rows succeeded</span>
                    : <span className="flex items-center gap-1.5 text-xs font-medium text-red-400"><XCircle
                        size={13}/> {result.failureCount} row(s) failed</span>
                }
            </div>
            ...
            <div className="grid grid-cols-3 divide-x divide-slate-700">
                {[
                    ...
                    {label: 'Succeeded', value: result.successCount, color: 'text-green-400'},
                    {
                        ...
                        color: result.failureCount > 0 ? 'text-red-400' : 'text-slate-500'
                    },
